...
$(document).ready(function(){
	...
	if (navigator.userAgent.indexOf('iPhone') > 0 || navigator.userAgent.indexOf('Android') > 0) {
		...
		if(rnd >= 1 && rnd <= 20){
			$("#txtsearch").attr("placeholder", "keyword search - space separated");
		}
		if(rnd >= 21 && rnd <= 40){
			$("#txtsearch").attr("placeholder", "keyword search - add by clicking on the card list");
		}
		if(rnd >= 41 && rnd <= 60){
			$("#txtsearch").attr("placeholder", "keyword search - remove by clicking on the deck list");
		}
		if(rnd >= 61 && rnd <= 80){
			$("#txtsearch").attr("placeholder", "keyword search - reduce number by right-clicking on the deck list");
		}
		if(rnd >= 101 && rnd <= 110){
			$("#txtsearch").attr("placeholder", "keyword search - 「book」 選択カードのみ表示");
		}
		if(rnd >= 111 && rnd <= 120){
			$("#txtsearch").attr("placeholder", "keyword search - 「NSR」レアリティ選択");
		}
		if(rnd >= 121 && rnd <= 130){
			$("#txtsearch").attr("placeholder", "keyword search - 「fav」 お気に入りのみ表示");
		}
		if(rnd >= 131 && rnd <= 140){
			$("#txtsearch").attr("placeholder", "keyword search - 右クリックで★お気に入り");
		}
		if(rnd >= 141 && rnd <= 150){
			$("#txtsearch").attr("placeholder", "keyword search - 「st=40 hp=40」でミノとか");
		}
		if(rnd >= 151 && rnd <= 160){
			$("#txtsearch").attr("placeholder", "keyword search - 「st>60」でST60以上だ");
		}
		if(rnd >= 161 && rnd <= 170){
			$("#txtsearch").attr("placeholder", "keyword search - 「クリーチャー」でクリーチャー");
		}
		if(rnd >= 171 && rnd <= 180){
			$("#txtsearch").attr("placeholder", "keyword search - 「火水」で火と水クリーチャー");
		}
		if(rnd >= 301 && rnd <= 302){
...
function CreateBtn(id, flg){
	...
	var btn = $('<div>');
	btn.addClass("card");
	btn.css("background-image", "url('img/cards/"+card.img+".jpg')");
	if(flg == "set"){
		btn.attr("id",card.id);
	}
	if(flg == "simu"){
...
function CreateBookImage() {
	if(Book.length < 1){
		alert("Please add at least 1 card.");
	}else {
		var BookImg = [];
		var url_plus = '';
		var cnt = {"C": 0, "I": 0, "S": 0}
		...
		//Tweet Strings
		var comment = "Card ratio " + cnt.C + ":" + cnt.I + ":" + cnt.S;
		deckurlplus = " http://www.culdceptcentral.com/deckbuilder/cr3dsb2p/?" + url_plus;
...
function Tweet(){
	...
	if(localStorage.twky1){
		$.post("http://aa1.versus.jp/revolt/tweet.php",{u1:localStorage.twky1, u2:localStorage.twky2, png:sendimg, cmt:comment});
	}else {
		window.open("http://aa1.versus.jp/revolt/twoauth.php", "new");
	}
...
function IMGWRITE(id, cnt, x ,y) {
	...
	img.src = "img/cards/" + card.img + ".jpg";
}
//コード変換機
function N2N4(n2){
